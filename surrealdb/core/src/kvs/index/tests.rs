...
/// A `DEFINE INDEX` in the same transaction as the record writes (a `{ }`
/// block runs in one transaction) must not publish an empty index. The build
/// is deferred: the statement stages an ownerless `Building` state atomically
/// with the definition and the data, and the post-commit start builds the
/// index from the committed records (see issue #637).
#[tokio::test(flavor = "multi_thread")]
async fn define_index_same_transaction_records_defers_build() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(
		&ds,
		&session,
		"{
			CREATE |user:100| SET email = 'a@example.com' RETURN NONE;
			DEFINE INDEX test ON user FIELDS email;
		}",
	)
	.await?;
	// Whether or not the deferred build has completed yet, queries must be
	// correct: a `Building` index is not used by the planner.
	expect_indexed_query_len(
		&ds,
		&session,
	...
	let building = index_building_json(&ds, &session, "user", "test").await?;
	assert_eq!(
		building.get("initial").and_then(|v| v.as_u64()),
		Some(100),
		"the deferred initial scan should have indexed every committed record: {building}"
	);
	expect_indexed_query_len(
		&ds,
		&session,
		"SELECT * FROM user WITH INDEX test WHERE email = 'a@example.com'",
		100,
	)
	.await?;
	Ok(())
...
/// Writes issued after a deferred `DEFINE INDEX` in the same transaction skip
/// admission (no durable state exists to reserve against) and are covered by
/// the deferred initial scan; no stray queue entries may remain.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_deferred_build_covers_later_writes_in_transaction() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(
		&ds,
		&session,
		"{
			CREATE user:one SET email = 'a@example.com' RETURN NONE;
			DEFINE INDEX test ON user FIELDS email;
			CREATE user:two SET email = 'a@example.com' RETURN NONE;
			UPDATE user:one SET email = 'b@example.com';
			CREATE user:gone SET email = 'a@example.com' RETURN NONE;
			DELETE user:gone;
		}",
	)
	.await?;
	...
	assert_eq!(
		building.get("initial").and_then(|v| v.as_u64()),
		Some(2),
		"only the two surviving records should be scanned: {building}"
	);
	expect_indexed_query_len(
		&ds,
		&session,
	...
	expect_indexed_query_len(
		&ds,
		&session,
		"SELECT * FROM user WITH INDEX test WHERE email = 'b@example.com'",
		1,
	)
	.await?;
	assert_eq!(
		durable_queue_all_generations_count(&ds, &ikb).await?,
		0,
		"deferred-build writers must not leave queued mutations or reservations"
	);
	Ok(())
}

/// A cancelled transaction discards the staged definition, the staged build
/// state, and the deferred start: nothing durable remains and no build runs.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_deferred_build_rolls_back_with_transaction() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_cancelled_transaction(
		&ds,
		&session,
		"BEGIN;
		CREATE |user:10| SET email = 'a@example.com' RETURN NONE;
		DEFINE INDEX test ON user FIELDS email;
		CANCEL;",
	)
	.await?;
	// Give any (incorrectly) spawned deferred start time to run.
	sleep(Duration::from_millis(200)).await;
	let tx = ds.transaction(TransactionType::Read, Optimistic).await?;
	let ns = catch!(tx, tx.get_ns_by_name("test", None).await).expect("namespace should exist");
	let db =
	...
	let table = TableName::from("user");
	let ix =
		catch!(tx, tx.get_tb_index(ns.namespace_id, db.database_id, &table, "test", None).await);
	tx.cancel().await?;
	assert!(ix.is_none(), "the cancelled definition must not be visible");
	// Redefining on committed data still builds synchronously and correctly.
	execute_all(
		&ds,
		&session,
		"CREATE |user:5| SET email = 'a@example.com' RETURN NONE;
		DEFINE INDEX test ON user FIELDS email;",
	)
	.await?;
	assert_eq!(index_building_status(&ds, &session, "user", "test").await?, "ready");
...
/// A transaction with only schema writes (no record writes) keeps the
/// synchronous blocking build: several `DEFINE INDEX`es in one block over
/// committed data are all ready the moment the block returns.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_schema_only_transaction_builds_synchronously() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(&ds, &session, "CREATE |user:20| SET email = 'a@example.com' RETURN NONE;").await?;
	// The leading DEFINE SEQUENCE clears its `!ba`/`!st` spans in this same
	// transaction; those record-free deletes must not defer the builds.
	execute_all(
		&ds,
		&session,
		"{
			DEFINE SEQUENCE seq;
			DEFINE INDEX test ON user FIELDS email;
			DEFINE INDEX test2 ON user FIELDS email, id;
		}",
	)
	.await?;
	// No wait: the blocking builds completed inside the block.
	assert_eq!(index_building_status(&ds, &session, "user", "test").await?, "ready");
	assert_eq!(index_building_status(&ds, &session, "user", "test2").await?, "ready");
	expect_indexed_query_len(
		&ds,
		&session,
		"SELECT * FROM user WITH INDEX test WHERE email = 'a@example.com'",
...
/// Record writes to *other* tables in the same transaction must not defer
/// the build: the initial scan only reads the indexed table's record span.
/// Mirrors fixture imports that populate one table before indexing another
/// (e.g. `access/record/knn_select_permission_import.surql`), which rely on
/// the index being ready — and covering their later same-transaction
/// inserts — the moment the block returns.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_other_table_writes_stay_synchronous() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(
		&ds,
		&session,
		"{
			CREATE other:1 SET name = 'fixture' RETURN NONE;
			DEFINE INDEX test ON user FIELDS email;
			CREATE user:one SET email = 'a@example.com' RETURN NONE;
			CREATE user:two SET email = 'a@example.com' RETURN NONE;
		}",
	...
	.await?;
	// No wait: the blocking build completed inside the block (the indexed
	// table had no staged writes when DEFINE INDEX ran), and the writes
	// after it indexed normally against the durably-Online build state.
	assert_eq!(index_building_status(&ds, &session, "user", "test").await?, "ready");
	expect_indexed_query_len(
		&ds,
		&session,
...
/// A schema-only migration transaction (`REMOVE INDEX` then `DEFINE INDEX`)
/// keeps the synchronous blocking build: retiring the old index deletes its
/// `!bg`/`!bp`/`!br` queue spans (and possibly the table's `!di`/`!dd`
/// doc-ID mappings), but those are typed record-free prefixes and must not
/// count as record writes. In particular, a UNIQUE build over committed
/// duplicates must fail the statement synchronously — rolling back the whole
/// migration — rather than commit and fail asynchronously.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_after_remove_index_stays_synchronous() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(
		&ds,
		&session,
	...
	.await?;
	// Replacing the index in one schema-only block stays synchronous: the
	// new index is ready and populated the moment the block returns.
	execute_all(
		&ds,
		&session,
		"{
			REMOVE INDEX test ON user;
			DEFINE INDEX test2 ON user FIELDS email;
		}",
	)
	.await?;
	assert_eq!(index_building_status(&ds, &session, "user", "test2").await?, "ready");
	expect_indexed_query_len(
		&ds,
		&session,
		"SELECT * FROM user WITH INDEX test2 WHERE email = 'a@example.com'",
		2,
	)
	.await?;
	// A UNIQUE index over committed duplicates fails the blocking build
	// inside the transaction, so the whole migration rolls back: the old
	// index survives and the new one never exists.
	let results = ds
		.execute(
			"{
				REMOVE INDEX test2 ON user;
				DEFINE INDEX uniq ON user FIELDS email UNIQUE;
			}",
			&session,
			None,
		)
	...
	let error = results
		.into_iter()
		.find_map(|result| result.result.err())
		.expect("the unique build over duplicates should fail the block");
	assert!(
		error.to_string().contains("already contains"),
		"expected a duplicate-entry build failure, got: {error}"
	);
	assert_eq!(
		index_building_status(&ds, &session, "user", "test2").await?,
		"ready",
		"the rolled-back migration must leave the previous index in place"
	);
	let tx = ds.transaction(TransactionType::Read, Optimistic).await?;
	let ns = catch!(tx, tx.get_ns_by_name("test", None).await).expect("namespace should exist");
	let db =
	...
	let table = TableName::from("user");
	let uniq =
		catch!(tx, tx.get_tb_index(ns.namespace_id, db.database_id, &table, "uniq", None).await);
	tx.cancel().await?;
	assert!(uniq.is_none(), "the failed unique index must not be visible");
	Ok(())
}

/// Record-write tracking is savepoint-aware: a failed create attempt
/// (`INSERT IGNORE` on an existing id) stages nothing once its savepoint
/// rolls back, so it must not defer a later `DEFINE INDEX` in the same
/// transaction — while marks from writes staged *before* (or outside) the
/// savepoint must survive the rollback.
#[tokio::test(flavor = "multi_thread")]
async fn define_index_after_rolled_back_insert_stays_synchronous() -> Result<()> {
	let (ds, session) = new_index_test_ds().await?;
	execute_all(&ds, &session, "CREATE user:one SET email = 'a@example.com' RETURN NONE;").await?;
	// The ignored insert's create attempt marks the table before the engine
	// write fails; the savepoint rollback must clear that mark, keeping the
	// define synchronous — ready and populated the moment the block returns.
	execute_all(
		&ds,
		&session,
		"{
			INSERT IGNORE INTO user { id: user:one, email: 'x@example.com' };
			DEFINE INDEX after_ignore ON user FIELDS email;
		}",
	)
	.await?;
	assert_eq!(index_building_status(&ds, &session, "user", "after_ignore").await?, "ready");
	expect_indexed_query_len(
		&ds,
		&session,
		"SELECT * FROM user WITH INDEX after_ignore WHERE email = 'a@example.com'",
		1,
	)
	.await?;
	// A real record write before the rolled-back attempt must keep deferring:
	// the deferred initial scan sees both committed records (initial = 2),
	// whereas a synchronous build would have scanned only user:one.
	execute_all(
		&ds,
		&session,
		"{
			CREATE user:real SET email = 'b@example.com' RETURN NONE;
			INSERT IGNORE INTO user { id: user:one, email: 'x@example.com' };
			DEFINE INDEX after_mixed ON user FIELDS email, id;
		}",
	)
	.await?;
	wait_for_index_ready(&ds, &session, "user", "after_mixed").await?;
	let building = index_building_json(&ds, &session, "user", "after_mixed").await?;
	assert_eq!(
		building.get("initial").and_then(|v| v.as_u64()),
		Some(2),
		"the surviving record write must still defer the build: {building}"
	);
	Ok(())
}

/// Multi-node: a deferred build completed by the defining node must be fully
/// visible from every other compute node sharing the storage engine — the
/// whole handshake (staged definition + ownerless `Building` state, adoption,
/// `Online` publication, index entries) lives in durable keys, never in
/// process-local state.
#[cfg(feature = "kv-mem")]
#[tokio::test(flavor = "multi_thread")]
async fn deferred_build_result_visible_from_other_nodes() -> Result<()> {
	let (ds_a, ds_b, session) = new_distributed_index_test_ds().await?;
	execute_all(
		&ds_a,
		&session,
	...
	.await?;
	// Wait for readiness as observed by the OTHER node.
	wait_for_index_ready(&ds_b, &session, "user", "test").await?;
	let building = index_building_json(&ds_b, &session, "user", "test").await?;
	assert_eq!(building.get("initial").and_then(|v| v.as_u64()), Some(2));
	expect_indexed_query_len(
		&ds_b,
		&session,
		"SELECT * FROM user WITH INDEX test WHERE email = 'a@example.com'",
	...
	.await?;
	// And writes from the other node maintain it.
	execute_all(&ds_b, &session, "CREATE user:three SET email = 'a@example.com' RETURN NONE;")
		.await?;
	expect_indexed_query_len(
		&ds_a,
		&session,
...
/// Multi-node: if the defining node dies after committing a deferred build
/// but before its post-commit start runs, the staged ownerless `Building`
/// state is immediately adoptable — `build_owner_expired` treats a missing
/// owner as expired, so ANOTHER node's stalled-build resume scan picks it up
/// on its next pass (no owner-lease wait) and drives it to `Online`.
#[cfg(feature = "kv-mem")]
#[tokio::test(flavor = "multi_thread")]
async fn deferred_build_orphaned_by_dead_node_adopted_by_resume_scan() -> Result<()> {
	let (ds_a, ds_b, session) = new_distributed_index_test_ds().await?;
	// Reconstruct the exact durable footprint a crashed defining node leaves
	// behind: committed records, a committed index definition, an ownerless
	// `Building` state, and NO index data. (The definition is created over
	// the empty table first so its blocking build is a no-op, then the rows
	// are added, and the healthy `Online` state and any index data are
	// replaced with the staged-deferred shape.)
	execute_all(&ds_a, &session, "DEFINE INDEX test ON user FIELDS email;").await?;
	execute_all(
		&ds_a,
		&session,
		"CREATE user:one SET email = 'a@example.com' RETURN NONE;
	...
	let ikb = IndexKeyBase::new(ns, db, table.clone(), ix.index_id);
	let tx = ds_a.transaction(TransactionType::Write, Optimistic).await?;
	catch!(tx, tx.del(&ikb.new_bs_key()).await);
	let index_prefix = index_all::new(ns, db, &table, ix.index_id);
	catch!(tx, tx.delp(&index_prefix).await);
	tx.commit().await?;
	// Staged in a second transaction: `stage_deferred_index_build` uses an
	// insert-only put, which must see the old state's delete committed.
	let tx = ds_a.transaction(TransactionType::Write, Optimistic).await?;
	catch!(tx, stage_deferred_index_build(&tx, ns, db, &table, ix.index_id).await);
	tx.commit().await?;
	let state = durable_build_state(&ds_a, &ikb).await?;
	assert_eq!(state.phase, IndexBuildPhase::Building);
	assert!(state.owner.is_none(), "the staged deferred state must be ownerless");

	// Node B's resume scan adopts the ownerless generation on its first pass.
	let resumed = ds_b
		.resume_stalled_index_builds(
			Duration::from_secs(30),
			tokio_util::sync::CancellationToken::new(),
		...
		.await?;
	assert_eq!(resumed, 1, "node B should adopt the orphaned deferred build");
	let deadline = Instant::now() + Duration::from_secs(30);
	loop {
		let state = durable_build_state(&ds_b, &ikb).await?;
		if state.phase == IndexBuildPhase::Online {
			break;
		}
		assert!(
			Instant::now() < deadline,
			"adopted deferred build did not complete; phase={:?}",
			state.phase
		);
		sleep(Duration::from_millis(100)).await;
	}
	// Both nodes see the completed, populated index.
	for ds in [&ds_a, &ds_b] {
		let building = index_building_json(ds, &session, "user", "test").await?;
		assert_eq!(building.get("status").and_then(|s| s.as_str()), Some("ready"));
		expect_indexed_query_len(
			ds,
			&session,
			"SELECT * FROM user WITH INDEX test WHERE email = 'a@example.com'",
